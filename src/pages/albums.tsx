...
const PER_PAGE = 50
const types = ['recent', 'random', 'newest', 'frequent', 'starred', 'alphabeticalByName', 'alphabeticalByArtist']
const Albums = () => {
  const [selected, setSelected] = React.useState(0);
  const { data, size, setSize } = useSubsonicInfinite<SubsonicAlbumsResponse>(index => `${GET_ALBUMS}?type=${types[selected]}&size=${PER_PAGE}&offset=${index * PER_PAGE}`);
...
{/* TODO: use select from radix */}
    <select value={types[selected]} onChange={e => setSelected(types.indexOf(e.target.value))}>
      {types.map((type, i) => 
